...
from mcp import ErrorData, McpError # type: ignore
from mcp.server.auth.provider import AccessToken # type: ignore
from mcp.types import INTERNAL_ERROR, INVALID_PARAMS, TextContent # type: ignore
from pydantic import BaseModel, AnyUrl, Field # type: ignore
import readabilipy # type: ignore
from pathlib import Path

TOKEN = "474b893f0475"
MY_NUMBER = "917420868177"  # Your number {91}{Your number}
...
class Fetch:
    ...
    async def fetch_url(
        ...
        """
        from httpx import AsyncClient, HTTPError # type: ignore
...
mcp = FastMCP(
    "Balaji Ganesh Tammireddy MCP Server",
    auth=SimpleBearerAuthProvider(TOKEN),
)

ResumeToolDescription = RichToolDescription(
    description="Serve your resume in plain markdown format.",
    use_when="When Puch or anyone asks for your resume; this must return raw markdown with no extra formatting.",
    side_effects=None,
)
...
async def resume() -> str:
    ...
    This function reads a local file named 'resume.md' and returns its
    content as a string.
    """
    ...
    if not resume_file.is_file():
        raise McpError(ErrorData(code=INTERNAL_ERROR, message="Resume file 'resume.md' not found."))
    ...
    try:
        ...
        raise McpError(ErrorData(code=INTERNAL_ERROR, message=f"Failed to read resume file: {e}"))
...
async def main():
    print("Starting Balaji's MCP Server for Puch AI...")
